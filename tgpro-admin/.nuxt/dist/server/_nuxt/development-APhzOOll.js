...
var _sfc_main = {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(versionInfo).hasUpdate) _push(`<div class="flex justify-between"><span class="text-[#9ca3af]">更新大小</span><span class="text-white">${ssrInterpolate(formatFileSize(unref(versionInfo).updateInfo?.size))}</span></div>`);
			else _push(`<!---->`);
			_push(`</div>`);
			...
			if (unref(independentSteps).download.status === "in_progress" && unref(independentSteps).download.progress !== void 0) {
				...
				if (unref(independentSteps).download.logs && unref(independentSteps).download.logs.length > 0) _push(`<span>${ssrInterpolate(unref(independentSteps).download.logs?.at(-1)?.message || "下载中...")}</span>`);
				else _push(`<!---->`);
				_push(`</div></div>`);
			} else _push(`<!---->`);
...
//# sourceMappingURL=development-APhzOOll.js.map