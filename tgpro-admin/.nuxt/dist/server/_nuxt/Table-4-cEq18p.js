...
import { U as table_default, W as useUI } from "./ui-G7Oicn0a.js";
...
import { t as Progress_default } from "./Progress-BNi6gYl8.js";
import { computed, defineComponent, mergeProps, resolveComponent, toRaw, toRef, useSSRContext } from "vue";
...
var _sfc_main = defineComponent({
	...
	setup(props, { emit, attrs: $attrs }) {
		...
		const rows = computed(() => {
			if (!sort.value?.column || props.sortMode === "manual") return props.rows;
			const { column, direction } = sort.value;
			return props.rows.slice().sort((a, b) => {
				const aValue = get(a, column);
				const bValue = get(b, column);
				return (columns.value.find((col) => col.key === column)?.sort ?? defaultSort)(aValue, bValue, direction);
			});
		});
...
//# sourceMappingURL=Table-4-cEq18p.js.map