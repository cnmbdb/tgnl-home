...
const useFormGroup = (inputProps, config, bind = true) => {
	...
	if (formGroup) {
		if (!bind || inputProps?.legend) formGroup.inputId.value = void 0;
		else if (inputProps?.id) formGroup.inputId.value = inputProps?.id;
	...
	const emitFormInput = useDebounceFn(() => {
		if (blurred.value || formGroup?.eagerValidation.value) emitFormEvent("input", formGroup?.name.value);
	}, 300);
	return {
...
export { useFormGroup as t };

//# sourceMappingURL=useFormGroup-ZK-CpXpd.js.map