import { d as defineEventHandler, r as requireAdmin, c as createError, a as readBody, e as executeQuery, u as useRuntimeConfig } from '../../../nitro/nitro.mjs';
...
const activateOrder_post = defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (existingLicense.length > 0) {
      ...
      const currentServerIp = config2.SERVER_IP || "localhost";
      try {
        const wpResponse = await $fetch(`${config2.WORDPRESS_URL}/wp-json/zibll/v1/verify-server`, {
          method: "POST",
          headers: {
            "Authorization": `Basic ${Buffer.from(`${config2.WORDPRESS_USERNAME}:${config2.WORDPRESS_APP_PASSWORD}`).toString("base64")}`,
            "Content-Type": "application/json"
          },
          body: {
