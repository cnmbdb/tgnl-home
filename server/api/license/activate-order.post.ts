...
export default defineEventHandler(async (event) => {
  ...
  let clientIp = headers['x-forwarded-for']?.split(',')[0].trim() 
    || headers['x-real-ip'] 
    || event.node.req.socket.remoteAddress 
    || 'localhost'
  
  // 处理IPv6的localhost (::1 -> localhost)
  ...
  try {
    ...
    if (existingLicense.length > 0) {
      ...
      // 使用WordPress插件的verify-server接口验证IP
      try {
        ...
        const wpResponse: any = await $fetch(`${config.WORDPRESS_URL}/wp-json/zibll/v1/verify-server`, {
          ...
          body: {
            ...
            server_ip: clientIp  // 使用动态获取的客户端IP
          }
        })
        ...
        throw createError({
          statusCode: 403,
          statusMessage: wpError.data?.message || '当前服务器IP未授权,请先在WordPress用户资料中绑定此服务器IP'
        })
      }
    }
    ...
    // 使用WordPress插件验证服务器IP是否已授权
    console.log('[激活] 验证服务器IP:', clientIp)
    try {
      const auth = Buffer.from(`${config.WORDPRESS_USERNAME}:${config.WORDPRESS_APP_PASSWORD}`).toString('base64')
      const verifyResponse: any = await $fetch(`${config.WORDPRESS_URL}/wp-json/zibll/v1/verify-server`, {
        method: 'POST',
        headers: {
          'Authorization': `Basic ${auth}`,
          'Content-Type': 'application/json'
      ...
      throw createError({
        statusCode: 403,
        statusMessage: verifyError.data?.message || '当前服务器IP未授权。请先访问WordPress网站,在个人资料中绑定此服务器IP (最多4个)'
      })
    }
    ...
    await executeQuery(
      ...
      [
        orderNumber,
        config.SERVER_IP as string || 'localhost',
